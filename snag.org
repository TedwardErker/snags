...
#+begin_src R
  ...
  shapefile(poly.extents.merged, "data/NAIPImages/extents.shp", overwrite = T)
      ...
      cropped.images <- lapply(6:length(projs), function(i) {             # skipping the black river, because they are already trimmed
          c.out.path <- paste0(tools::file_path_sans_ext(image.files[i]),"_cropped.tif")
          m.out.path <- paste0(tools::file_path_sans_ext(image.files[i]),"_masked.tif")
          if(grepl(".*zone=15.*", projs[i])) {
            ci <- crop(images[[i]], bufs$utm15, c.out.path, overwrite = T)
            mask(ci, bufs$utm15, m.out.path, overwrite = T)
          }
          if(grepl(".*zone=16.*", projs[i])) {
            ci <- crop(images[[i]], bufs$utm16, c.out.path, overwrite = T)
            mask(ci, bufs$utm16, m.out.path, overwrite = T)
          }
      })
#+end_src
