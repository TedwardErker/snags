...
| [2018-10-03 Wed 10:00] | [2018-10-03 Wed]       |       |   |   |
...
#+begin_src R
  ...
  x[x < mnx] <- mnx
        ...
        if (statistics == "correlation") {
            ...
            texture <- rescale.0.254(texture, 0, 1)
        } else {
            texture <- glcm(x, window = window, statistics = statistics, shift = shift, na_opt = na_opt, na_val = na_val, min_x = mnx, max_x =mxx)
...
#+RESULTS:
: Error in dnirname(tile.path) : could not find function "dnirname"
...
#+begin_src R
  ...
  text <- str_extract(f, ".*stat.*") %>% na.omit()
  f <- c(conv, text)
  train.stack <- stack(f)
