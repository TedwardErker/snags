...
| [2018-12-21 Fri 9:20]  | [2018-12-21 Fri    ]   |       |                  |                                                      |
...
p <- spTransform(d, CRS("+proj=tmerc +lat_0=0 +lon_0=-90 +k=0.9996 +x_0=520000 +y_0=-4480000 +ellps=GRS80 +units=m +no_defs"))
  ...
  bufs$wtm <- disaggregate(buf)
    ...
    shapefile(bufs$wtm, "data/NAIPImages/MYSE_captures_2014_utm16.shp", overwrite = T)
#+end_src
...
#+begin_src R
  ...
  image.files <- list.files("data/NAIPImages", recursive = T, full.names = T, pattern = ".*[0-9]{8}[_w]*.tif$")[19]
