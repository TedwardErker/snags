...
#+begin_src R
  ...
  tile.dirs <- tile.dirs[grepl(".*[0-9]{8}[_w]*", tile.dirs)]
    ...
    lapply(tile.dirs[c(19,21,23)], function(tile.dir) {
...
#+begin_src R
  library(tidyr)
    ...
    tile.dirs <- tile.dirs[2:length(tile.dirs)]
      ...
      lapply(tile.dirs[20], function(tile.dir) {
          tile.names <- tools::file_path_sans_ext(list.files(tile.dir, pattern = ".*masked_[0-9]+.tif$"))
...
#+begin_src R
  ...
  lapply(imgs, function(img) {
      ...
      f <- list.files(paste0(f.dir, img), full.names = T, pattern = "masked_12_.*.tif$")  #I changed the number to 12 because there iis also an "11" for this tile
  ...
  features <- foreach (img = imgs[c(3,4)],
                       .packages = c("raster","stringr","randomForest")) %dopar% {
