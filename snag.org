...
stretch.vals <- cbind(mns,mxs)
write.csv(stretch.vals, "data/training/stretchvals.csv")
#+END_SRC
...
#+begin_src R
    ...
    liveveg.cells <- sample(which(getValues(liveveg == 255)),1000)
    liveveg.df <- data.frame(cell = liveveg.cells, Class = "liveveg")

    livetree.cells <- sample(which(getValues(livetree == 255)),1000)
    livetree.df <- data.frame(cell = livetree.cells, Class = "livetree")

    other.cells <- sample(which(getValues(other == 255)),1000)
...
#+begin_src R
stretch.vals <- read.csv(stretch.vals, "data/training/stretchvals.csv")
...
#+begin_src R
pred.test <- predict(test.stack, mod)
#+end_src
