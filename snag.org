...
#+begin_src R
  ...
  d <- read_excel("data/NAIPImages/MYSE_captures_2014_new.xlsx")
  coordinates(d) <- ~long + lat
  proj4string(d) <- CRS("+init=epsg:4326")

  shapefile(d, "data/NAIPImages/MYSE_captures_2014_new.shp", overwrite = T)
...
#+begin_src R
  ...
  cropped.images <- lapply(c(19,25), function(i) {
...
#+begin_src R
  ...
  tile.dirs <- tile.dirs[grepl(".*[0-9]{8}[_w]*", tile.dirs)]
    ...
    lapply(tile.dirs[18:25], function(tile.dir) {
...
: 
:     extract
: Error in remove.tif.ext(list.files(tile.dir, paste0(tile.name, focal.feature.pattern))) : 
:   could not find function "remove.tif.ext"
...
#+begin_src R
  #m_4509025_ne_15_1_20130703
    ...
    cl <- makeCluster(cores)
      ...
      features <- foreach (img = imgs[c(21,22)],
                           .packages = c("raster","stringr","randomForest")) %dopar% {
