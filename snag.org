...
#+begin_src R
  ...
  shapefile(d, "data/NAIPImages/MYSE_captures_2014.shp")
...
#+begin_src R
  ...
  cropped.images <- lapply(c(19,21), function(i) {
...
#+begin_src R
  ...
  tile.dirs <- tile.dirs[grepl(".*[0-9]{8}[_w]*", tile.dirs)]
    ...
    lapply(tile.dirs[15], function(tile.dir) {
...
#+begin_src R
  library(tidyr)
    ...
    tile.dirs <- tile.dirs[2:length(tile.dirs)]
      ...
      lapply(tile.dirs[14], function(tile.dir) {
          tile.names <- tools::file_path_sans_ext(list.files(tile.dir, pattern = ".*masked_[0-9]+.tif$"))
