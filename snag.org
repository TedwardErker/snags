...
#+begin_src R
  ...
  cropped.images <- lapply(c(23,27), function(i) {
...
#+begin_src R
  ...
  tile.dirs <- tile.dirs[grepl(".*[0-9]{8}[_w]*", tile.dirs)]
    ...
    lapply(tile.dirs[c(23,27)], function(tile.dir) {
...
#+begin_src R
  ...
  mod <- readRDS(paste0(f.dir, "mod.rds"))
    ...
    cl <- makeCluster(cores)
      ...
      features <- foreach (img = imgs[c(8,9,10)],
                           .packages = c("raster","stringr","randomForest")) %dopar% {
...
#+begin_src R
  ...
  features <- foreach (img = imgs[c(5,6,10,11,12)],
                       .packages = c("raster","stringr","randomForest")) %dopar% {
...
#+begin_src R
  ...
  features <- foreach (img = imgs[c(10,11,12)],
                       .packages = c("raster","stringr","randomForest")) %dopar% {
